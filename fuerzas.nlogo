...
globals[
  A ;Parametro A que interviene
  B ;Parametro B que interviene
  m ;Masa de cada tortuga
  f ;Fuerza aplicada
  K
  kc
]
...
to crea-obstaculo
  ...
  ask patch 0 (max-pycor - 1) [set pcolor red]

  ; Generacion de agente
...
to crea-persona
  ...
  set B 0.08 ;m
  set m 80 ;kg
  set K 1.2 * 10 ^ 5 ;kg/s^2
  set kc 2.4 * 10 ^ 5 ;kg/m s
...
;El calculo de las fuerzas se divide en 3 terminos independientes para facilitar la comprension y su calculo

to fuerza
  ask turtles [; DEBERIA IR AL FINAL ESTE TERMINO
    set v list (xcor - item 0 p) (ycor - item 1 p)
    set p list xcor ycor
  ] ;Actualizamos

  ;Fuerza entre 1 y 0 en direccion a 0 sobre 1
  ask turtle 1
  [
    ask turtle 0
    [
      ...
      [ set termino_2 list (item 0 n_ij * k * r_d) (item 1 n_ij * k * r_d) ]
      ...
      set termino_3 list (kc * r_d * item 0 dv_ji * item 0 t_ij) (kc * r_d * item 1 dv_ji * item 1 t_ij)
...
;      show r_ij
      ...
      set f (modulo termino_1) + (modulo termino_2) + (modulo termino_3)
    ]
  ]
  ;tick
end
...
to-report muro?
  ifelse patch-ahead 1 != nobody [ report [pcolor] of patch-ahead 1 = violet ][ report true ]
end
@#$#@#$#@
GRAPHICS-WINDOW
202
