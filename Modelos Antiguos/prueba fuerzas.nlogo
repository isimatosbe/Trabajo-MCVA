...
to-report move
  ...
  report list ( m_c * (item 0 w_pos_media - item 0 p) + m_s * item 0 x_separation + m_l * (item 0 y_aligment - item 0 v) + 0 * item 0 z_obstacles ) ( m_c * (item 1 w_pos_media - item 1 p) + m_s * item 1 x_separation + m_l * (item 1 y_aligment - item 1 v) + 0 * item 1 z_obstacles )
end
...
to-report panico
  ...
  let nuevo_panico (d1 + d2 + d3 + d4) / 4 ; ¡¡¡¡¡¡¡¡¡¡¡Cambiar y dividir entre 4!!!!!!!!!!!!!
  ;print (panic + nuevo_panico) / 2
...
; Fuerzas de interacción entre agentes
  ...
  [
    ...
    [ ; Primer término de la fuerza entre dos agentes
      set termino_1 list 0 0;(A * e ^ (r_d / B) * item 0 n_ij) (A * e ^ (r_d / B) * item 1 n_ij)
    ]
    [
...
;    set m_o 3.5
    ...
    ; set v (map + (map [ i -> i * panic ] v_nueva) (map [ i -> i * (1 - panic) ] v))
  ...
  ask personas [
    ...
    if modulo_fuerza_total > 0 [print modulo_fuerza_total]
    ...
    set aceleracion (map + (aceleracion_recibe) (map [ i -> i / 0.5 ] (map - v_nueva v)))
    ...
    if (item 0 p) >= max-pxcor - 0.5 [set p list (max-pxcor - 1) (item 1 p)]
    if (item 1 p) >= max-pycor - 0.5 [set p list (item 0 p) (max-pycor - 1)]
    if (item 0 p) <= 0.5 - max-pxcor [set p list (- max-pxcor + 1) (item 1 p)]
    if (item 1 p) <= 0.5 - max-pycor [set p list (item 0 p) (- max-pycor + 1)]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot (numero_personas - count personas)"
